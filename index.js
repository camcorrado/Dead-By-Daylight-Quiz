...
function submitAnswer() {
  $(".questionAnswerForm").on("click", ".submitButton", function () {
    ...
    let answerChosen = document.querySelector(
      "input[name=answerOption]:checked"
    );
    let selected = $("input:checked");
    let answer = selected.val();
    let questionNumber =
      Number(document.getElementById("questionNumber").innerHTML) - 1;
...
function presentImagery() {
  ...
  $(".imagery").html(
    `<img src='${imagery}' alt='${alt}' crossorigin='anonymous'>`
  );
  $(".imagery").fadeIn(1000);
  if (audio2 === "null") {
    $(".audio").html(`
